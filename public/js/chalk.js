...
function chalkboard(){
	$('#chalkboard').remove();
	$('.chalk').remove();
    $('body').prepend('<a href="https://github.com/DaniAkash/Live-Chalkboard/fork"><img style="position: absolute; top: 0; right: 0; border: 0;height: 175px;width: 175px;z-index: 99999;" src="https://camo.githubusercontent.com/52760788cde945287fbb584134c4cbc2bc36f904/68747470733a2f2f73332e616d617a6f6e6177732e636f6d2f6769746875622f726962626f6e732f666f726b6d655f72696768745f77686974655f6666666666662e706e67" alt="Fork me on GitHub" data-canonical-src="https://s3.amazonaws.com/github/ribbons/forkme_right_white_ffffff.png"></a>');
	...
	function draw(x,y){
        socket.emit('draw', [x,y,xLast,yLast]);
    ...
    function remoteDraw(x,y,xl,yl){
        ...
        // Chalk Effect
        var length = Math.round(Math.sqrt(Math.pow(x-xl,2)+Math.pow(y-yl,2))/(5/brushDiameter));
        var xUnit = (x-xl)/length;
        var yUnit = (y-yl)/length;
        for(var i=0; i<length; i++ ){
            var xCurrent = xl+(i*xUnit);
            var yCurrent = yl+(i*yUnit);
            var xRandom = xCurrent+(Math.random()-0.5)*brushDiameter*1.2;
            var yRandom = yCurrent+(Math.random()-0.5)*brushDiameter*1.2;
            ctx.clearRect( xRandom, yRandom, Math.random()*2+2, Math.random()+1);
            }
    ...
    socket.on('remote draw', function(coord){
        remoteDraw(coord[0],coord[1],coord[3],coord[4]);
    });
    ...
    function remoteErase(x,y){
        ctx.clearRect (x-0.5*eraserWidth,y-0.5*eraserHeight,eraserWidth,eraserHeight);
    }

    socket.on('remote erase', function(coord){
        remoteErase(coord[0],coord[1]);
    });
